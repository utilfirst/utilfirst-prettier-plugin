...
- Pin every third-party GitHub Actions `uses:` to a full-length commit SHA, with a trailing `# vX.Y.Z` comment for readability and Dependabot updates (a tag can be repointed at malicious code, the entry point for the 2025-2026 supply-chain attacks, while a SHA cannot)
...
- Resolve an action's pin from a live source rather than memory (`gh api repos/OWNER/REPO/releases/latest` for the current major, `gh api repos/OWNER/REPO/commits/TAG --jq .sha` for the SHA)
